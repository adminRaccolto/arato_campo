...
type ProdutoVinculado = { insumo_id: string; dose_kg_ha: number; nome: string };
...
export function FechamentoAdubacao({
    ...
    async function carregar() {
      ...
      const [{ data: recData, error: recError }, { data: talhoesData }, { data: produtosData }] = await Promise.all([
        sb
          ...
          .select("id, ciclo_id, data_aplicacao_indicada, hectares_sugeridos, modalidade, profundidade_aplicacao_cm")
          .eq("id", recomendacaoId)
          .limit(1),
        sb.from("recomendacoes_adubacao_talhoes").select("talhao_id, area_ha, talhoes(nome)").eq("recomendacao_id", recomendacaoId),
        sb.from("recomendacoes_adubacao_produtos").select("insumo_id, dose_kg_ha, insumos(nome)").eq("recomendacao_id", recomendacaoId),
      ]);
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setSalvando(true);
    const sb = supabase as unknown as { from: (table: string) => ReturnType<typeof supabase.from> };
    const adubacaoId = crypto.randomUUID();
    const talhaoUnico = talhoes.length === 1 ? talhoes[0].talhao_id : null;

    const { error: insertError } = await supabase.from("adubacoes_base").insert({
      id: adubacaoId,
    ...
    if (produtos.length > 0) {
      await supabase.from("adubacoes_base_itens").insert(
        produtos.map((p) => ({
          id: crypto.randomUUID(),
          adubacao_id: adubacaoId,
          fazenda_id: fazendaId,
          insumo_id: p.insumo_id,
          produto_nome: p.nome,
          dose_kg_ha: p.dose_kg_ha,
        }))
      );
    }

    await sb.from("recomendacoes_adubacao").update({ hectares_realizados: Number(hectaresRealizados), data_aplicacao_realizada: dataRealizada }).eq("id", recomendacao.id);
